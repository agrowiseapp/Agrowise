...
class GoogleAuthExpoOfficial {
  ...
  createAuthRequest() {
    const request = new AuthSession.AuthRequest({
      ...
      state: `auth_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`, // Force fresh state
      additionalParameters: {
        prompt: "select_account", // Force account selection
        access_type: "offline",
        include_granted_scopes: "false", // Don't include previously granted scopes
        max_age: "0", // Force fresh authentication
        hd: "", // Clear hosted domain filter
      },
  ...
  /**
   * Clear any existing browser sessions/cookies
   */
  async clearBrowserSession() {
    try {
      // Clear any existing browser sessions
      await WebBrowser.dismissBrowser();
      console.log("🧹 Browser session cleared before sign-in");
    } catch (error) {
      console.log("⚠️ Browser session clear failed (non-critical):", error.message);
    }
  }
  ...
  async signIn() {
    try {
      ...
      // Clear any existing browser sessions first
      ...
      // Prompt for authentication - always show account picker
      const result = await request.promptAsync(this.discovery, {
        showInRecents: true,
        preferEphemeralSession: true, // Force account selection by not persisting cookies
      });
  ...
  async signOut() {
    try {
      // Get tokens before clearing them for revocation
      ...
      if (tokens && tokens.accessToken) {
        try {
          ...
          console.log("🔓 Access token revoked:", revokeResponse.ok ? "✅" : "❌");
        } catch (revokeError) {
          console.log("⚠️ Token revocation failed (non-critical):", revokeError.message);
        }
      }

      // Clear browser cookies/session data more aggressively
      try {
        await WebBrowser.dismissBrowser();
        console.log("🧹 Browser session dismissed");
      } catch (dismissError) {
        console.log("⚠️ Browser dismiss failed (non-critical):", dismissError.message);
      }
